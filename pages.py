  ...
  <div class="modal-card">
    ...
    <div class="modal-body">
      <form id="create-user-form">
        ...
        <div class="form-row">
          <div class="form-field"><label><i class="ti ti-plug-connected"></i> پروتکل</label><select class="form-select" id="cu-protocol"><option value="vless">VLESS</option><option value="vmess">VMess</option><option value="trojan">Trojan</option><option value="shadowsocks">Shadowsocks</option></select></div>
...
<aside class="sidebar" id="sidebar">
  ...
  <div class="sidebar-nav">
    ...
    <div class="nav-item" data-page="nodes"><i class="ti ti-branch"></i> Node ها</div>
...
<main class="main">
  ...
  <section class="page" id="page-settings">
    ...
    <div class="table-wrap" style="margin-bottom:18px">
      <div style="padding:18px 22px;border-bottom:1px solid var(--border)">
        <div class="server-title" style="margin-bottom:0"><i class="ti ti-settings-cog"></i> تنظیمات پیشرفته (Advanced)</div>
      </div>
      <div style="padding:18px 22px">
        <div class="form-row">
          <div class="form-field">
            <label><i class="ti ti-server"></i> Server IP</label>
            <input class="form-input" id="adv-ip" placeholder="در حال بارگذاری..." readonly dir="ltr">
          </div>
          <div class="form-field">
            <label><i class="ti ti-map-pin"></i> Country</label>
            <div id="adv-country" style="display:flex;align-items:center;gap:8px;padding:10px 14px;border-radius:var(--radius);border:1.5px solid var(--border);background:rgba(255,255,255,.5);backdrop-filter:blur(8px)">
              <span id="adv-flag" style="font-size:20px"></span>
              <span id="adv-country-text" style="font-size:12.5px;color:var(--text-primary)">در حال بارگذاری...</span>
            </div>
          </div>
        </div>
        <div class="form-field" style="margin-top:12px">
          <label><i class="ti ti-key"></i> API Key (Node Secret)</label>
          <div style="display:flex;align-items:center;gap:8px">
            <input class="form-input" id="adv-apikey" placeholder="spdr_xxxxxxxx" readonly dir="ltr" style="flex:1">
            <button class="btn btn-ghost" onclick="copyApikey()"><i class="ti ti-copy"></i> کپی</button>
            <button class="btn btn-ghost" onclick="rotateApikey()"><i class="ti ti-refresh"></i> چرخش</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <section class="page" id="page-nodes">
    <div class="topbar">
      <div><div class="topbar-title"><i class="ti ti-branch"></i> Node ها</div><div class="topbar-sub">پنل‌های ریموت</div></div>
      <div class="topbar-right"><button class="btn btn-primary" onclick="openAddNodeModal()"><i class="ti ti-plus"></i> افزودن Node</button></div>
    </div>
    <div id="nodes-list"></div>
    <div class="empty-state" id="nodes-empty" style="display:none"><i class="ti ti-node"></i><p>هنوز Node ای اضافه نشده</p></div>
  </section>
  ...
  <div class="modal-overlay" id="modal-add-node">
    <div class="modal-card" style="max-width:480px">
      <div class="modal-head">
        <div class="modal-title"><i class="ti ti-branch-plus"></i> افزودن Node</div>
        <button class="modal-close" onclick="closeModal('modal-add-node')"><i class="ti ti-x"></i></button>
      </div>
      <div class="modal-body">
        <div class="form-field"><label><i class="ti ti-brand-namesake"></i> نام</label><input class="form-input" id="node-name" placeholder="نام Node (اختیاری)"></div>
        <div class="form-field"><label><i class="ti ti-world"></i> دامنه</label><input class="form-input" id="node-domain" placeholder="https://example.com" dir="ltr"></div>
        <div class="form-field"><label><i class="ti ti-key"></i> API Key</label><input class="form-input" id="node-apikey" placeholder="spdr_xxxxxxxx" dir="ltr"></div>
      </div>
      <div class="modal-footer">
        <button class="btn btn-ghost" onclick="closeModal('modal-add-node')">انصراف</button>
        <button class="btn btn-primary" onclick="addNode()"><i class="ti ti-link"></i> افزودن و تست اتصال</button>
      </div>
    </div>
  </div>
</main>
...
function switchPage(name){
  ...
  var loaders={users:loadUsers,configs:loadConfigs,traffic:loadTraffic,logs:loadLogs,dash:loadDashboard,settings:loadSettings,worker:loadWorker,nodes:loadNodes};
...
function renderUsersTable(links,tableId){
  ...
  tbody.innerHTML=links.map(function(l){
    ...
    var protoFa=l.protocol==='vless-ws'?'VLESS':l.protocol==='vmess'?'VMess':l.protocol==='trojan'?'Trojan':l.protocol==='shadowsocks'?'Shadowsocks':l.protocol||'VLESS';
...
async function loadSettings(){
  try{
    ...
    // Load Advanced settings (IP, Country, API Key)
...
async function loadAdvancedSettings(){
  try{
    var r=await authFetch('/api/me'),d=await r.json();
    document.getElementById('adv-ip').value=d.ip||'-';
    var flagEl=document.getElementById('adv-flag');
    var countryEl=document.getElementById('adv-country-text');
    if(d.flag){
      flagEl.textContent=d.flag;
    }else{
      flagEl.textContent='';
    }
    // Try to get country from flag or use a fallback
    var ccMap={'IR':'🇮🇷','US':'🇺🇸','DE':'🇩🇪','NL':'🇳🇱','FR':'🇫🇷','GB':'🇬🇧','JP':'🇯🇵','SG':'🇸🇬','CA':'🇨🇦','AU':'🇦🇺'};
    var cc=d.flag||'';
    var countryName=ccMap[cc]||'';
    if(countryName)countryEl.textContent=countryName;
    else countryEl.textContent='-';
    // Masked API key (first 8 chars + hidden)
    var key=d.api_key||'';
    if(key&&key.length>8){
      document.getElementById('adv-apikey').value=key.substring(0,8)+'********';
    }else{
      document.getElementById('adv-apikey').value=key;
    }
  }catch(e){console.error(e)}
}

function copyApikey(){
  var keyEl=document.getElementById('adv-apikey');
  var realKey=keyEl.getAttribute('data-real')||'';
  if(!realKey){
    toast('API Key در دسترس نیست','err');
    return;
  }
  navigator.clipboard.writeText(realKey).then(function(){
    toast('API Key کپی شد ✓','ok');
  }).catch(function(){
    toast('خطا در کپی','err');
  });
}

async function rotateApikey(){
  if(!confirm('آیا از چرخش API Key اطمینان دارید؟ این عملیات غیرقابل بازگشت است.'))return;
  try{
    var r=await authFetch('/api/settings/security-token/rotate',{method:'POST'});
    var d=await r.json();
    if(!d.ok)throw new Error(d.detail||'خطا');
    var key=d.security_token||'';
    if(key&&key.length>8){
      document.getElementById('adv-apikey').value=key.substring(0,8)+'********';
      document.getElementById('adv-apikey').setAttribute('data-real',key);
    }
    toast('API Key جدید تولید شد ✓','ok');
  }catch(e){toast(e.message||'خطا در چرخش API Key','err')}
}
...
async function loadNodes(){
  try{
    var r=await authFetch('/api/nodes'),d=await r.json();
    ...
    list.innerHTML=nodes.map(function(n){
      var statusClass=n.last_status==='online'?'badge-green':n.last_status==='connecting'?'badge-amber':'badge-red';
      var statusText=n.last_status==='online'?'متصل':n.last_status==='connecting'?'در حال اتصال':n.last_status==='invalid_key'?'API Key نامعتبر':'قطع';
      var apiKeyMasked=(n.api_key_masked||'').replace(/^(.{6}).*(.{4})$/,'$1…$2');
      return '<div class="table-wrap" style="margin-bottom:12px">'+
        '<div style="padding:16px 20px;display:flex;align-items:center;gap:16px;flex-wrap:wrap">'+
          '<div style="flex:1;min-width:0">'+
            '<div style="font-size:15px;font-weight:700;margin-bottom:6px">'+esc(n.name||n.domain)+'</div>'+
            '<div style="font-size:12px;color:var(--text-secondary);direction:ltr;text-align:left">'+esc(n.domain)+'</div>'+
            '<div style="font-size:11px;color:var(--muted);margin-top:4px">API Key: <code style="font-family:monospace">'+esc(apiKeyMasked)+'</code></div>'+
          '</div>'+
          '<div style="display:flex;flex-direction:column;align-items:flex-end;gap:6px">'+
            '<span class="badge '+statusClass+'">'+statusText+(n.latency_ms?' ('+n.latency_ms+'ms)':'')+'</span>'+
            (n.remote_ip?'<div style="font-size:11px;color:var(--muted)">'+(n.remote_flag||'')+' '+esc(n.remote_ip)+'</div>':'')+
            '<div style="display:flex;gap:6px">'+
              '<button class="btn btn-sm btn-ghost" onclick="testNodeConnection(\''+n.node_id+'\')"><i class="ti ti-refresh"></i> تست اتصال</button>'+
              '<button class="btn btn-sm btn-danger" onclick="deleteNode(\''+n.node_id+'\',\''+esc(n.name||n.domain)+'\')"><i class="ti ti-trash"></i> حذف</button>'+
            '</div>'+
          '</div>'+
        '</div>'+
      '</div>';
...
async function addNode(){
  ...
  var domain=document.getElementById('node-domain').value.trim();
  var api_key=document.getElementById('node-apikey').value.trim();
  if(!domain){toast('دامنه الزامی است','err');return}
  if(!api_key){toast('API Key الزامی است','err');return}
  try{
    var r=await authFetch('/api/nodes',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name,domain:domain,api_key:api_key})});
    var d=await r.json();
    if(!d.ok)throw new Error(d.detail||'خطا در افزودن نود');
    toast('Node با موفقیت اضافه شد ✓','ok');
    closeModal('modal-add-node');
    loadNodes();
  }catch(e){toast(e.message,'err')}
}

async function testNodeConnection(node_id){
  try{
    var r=await authFetch('/api/nodes/'+node_id+'/check',{method:'POST'});
    var d=await r.json();
    if(!d.ok)throw new Error(d.detail||'خطا');
    toast('تست اتصال انجام شد ✓','ok');
    loadNodes();
  }catch(e){toast(e.message,'err')}
}

async function deleteNode(node_id,node_name){
  if(!confirm('آیا از حذف Node «'+node_name+'» اطمینان دارید؟'))return;
  try{
    var r=await authFetch('/api/nodes/'+node_id,{method:'DELETE'});
    var d=await r.json();
    if(!d.ok)throw new Error(d.detail||'خطا در حذف');
    toast('Node حذف شد ✓','ok');
    loadNodes();
  }catch(e){toast(e.message,'err')}
}
...
/* ============ REFRESH ============ */
function refreshAll(){
  loadDashboard();loadUsers();loadConfigs();loadTraffic();loadLogs();loadWorker();loadNodes();
