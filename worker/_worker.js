...
// ══════════════════════════════════════════════════════════════════════════════
// Deployed by the panel to a Cloudflare Pages project in Advanced Mode. VLESS/WS/TLS tunnel.
//
// Route handling:
...
async function handleVlessWs(request, env, uuid) {
  ...
  server.addEventListener('message', async (ev) => {
    ...
    if (!server.__h) {
      ...
      if (!h || String(h.userId || '').toLowerCase() !== String(uuid || '').toLowerCase()) { try { server.close(4002, 'bad header'); } catch(e){} return; }
      server.__h = h;
